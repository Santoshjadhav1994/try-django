...
from django.forms.models import modelformset_factory # models form for queryset
from django.http import HttpResponse,Http404
from django.urls import reverse
# from .services import extract_text_via_ocr_services
...
def recipe_ingredient_delete_view(request,parent_id=None,id=None):
    try:
        obj = RecipeIngredient.objects.get(id=id,recipe_id=parent_id,recipe__user=request.user)
    except:
        obj = None
    if obj is None:
        if request.htmx:
    ...
    if request.method == "POST":
        ...
        if request.htmx:
            return render(request,"recipes/partials/ingredient-inline-delete.html",{'name':name})
        return redirect(success_url)
    context = {
        'object':obj
    }
...
def recipe_ingredient_image_upload_view(request, parent_id=None):
    ...
    if form.is_valid():
        ...
        # #################################  Microservices API for Text Extraction Images #################################
        # send image file --> microservices api
        # microservices api -> data about the file
        # 
        # result = extract_text_via_ocr_services(obj.image)
